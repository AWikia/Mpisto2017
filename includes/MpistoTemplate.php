 ...
 * @ingroup Skins
 */
class MpistoTemplate extends BaseTemplate {

	/**
	 * Template filter callback for Mpisto skin.
	...
	public function execute() {
		...
		$html .= Html::rawElement( 'div', [ 'id' => 'content', 'class' => 'mw-body',  'role' => 'main' ],
			...
			Html::rawElement( 'div', [ 'id' => 'bodyContent', 'class' => 'mw-body-content' ],
				...
				Html::element( 'a', [ 'href' => '#column-one', 'class' => 'mw-jump-link' ],
					$this->getMsg( 'mpisto-jumptonavigation' )->text()
				) .
				Html::element( 'a', [ 'href' => '#searchInput', 'class' => 'mw-jump-link' ],
					$this->getMsg( 'mpisto-jumptosearch' )->text()
				) .
				'<!-- start content -->' .
		...
		$html .= Html::rawElement( 'div',
			...
			$this->getMobileNavigationIcon(
				'p-personal',
				$this->getMsg( 'mpisto-jumptopersonal' )->text()
			) .
			$this->getMobileNavigationIcon(
				'globalWrapper',
				$this->getMsg( 'mpisto-jumptotop' )->text()
			)
		);
	...
	protected function getCactions() {
		...
		// Normal cactions
		if ( $tabCount > 2 ) {
			$html .= $this->getBox( 'cactions', $allTabs, 'mpisto-cactions-label' );
		} else {
		...
		if ( $tabCount !== 1 ) {
			...
			$tabs['more'] = [
				'text' => $this->getMsg( 'mpisto-more-actions' )->text(),
				'href' => '#p-cactions',
				'id' => 'ca-more'
			];
		}
		...
		$html .= $this->getBox( 'cactions-mobile', $tabs, 'mpisto-cactions-label' );
